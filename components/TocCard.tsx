...
export default function TocCard({ toc }: { toc: Toc }) {
    ...
    useEffect(() => {
        const updateActiveHeadings = () => {
            ...
            const currentMainHeading = headingPositions.reduce((prev, current) => {
    ...
    useEffect(() => {
        if (activeIds.length > 0 && tocRef.current) {
            ...
            const activeElement = tocContainer.querySelector(`a[href="#${activeIds[0]}"]`)?.parentElement
    ...
    return (
        <div className="sticky top-[50%] max-h-64 overflow-hidden prose max-w-none py-10 dark:prose-dark">
            {/* Top fade gradient */}
            <div className="absolute top-0 left-0 right-0 h-16 bg-gradient-to-b from-white to-transparent pointer-events-none z-10 dark:from-gray-950"></div>
            <div 
                ref={tocRef} 
                ...
                {tocElements.map((item) => {
                    ...
                    return (
                        <div className="flex items-center" key={item.url}>
                            ...
                            >
                                <path d={`M${item.prev * 4 - 2} 0V1L${item.depth * 4 - 2} 2V14L${item.depth * 4 - 2} 15v1`} strokeWidth={0.7}></path>
                            </svg>
                            <a 
                                className={`no-underline inline-block ml-3 max-h-8 text-nowrap overflow-x-hidden text-ellipsis transition-colors duration-200
                                    ${isActive ? 'text-orange-600 font-medium' : 'text-orange-600/40 dark:text-orange-900'}`} 
                                href={item.url}
                            >
                                {item.value}
                            </a>
            ...
            <div className="absolute bottom-0 left-0 right-0 h-16 bg-gradient-to-t from-white to-transparent pointer-events-none z-10 dark:from-gray-950"></div>
        </div>
    )
}
