...
Recently, I ran into an error that seemed to defy logic:
      ...
      <CodeBlock
        language="plaintext"
        code={`Error: Element type is invalid: expected a string (for built-in components) or a class/function (for composite components) but got: undefined.`}
      />
...
I used to display recommended tools using \`<a>\` elements, but today I decided to replace them with the \`<ToolCard>\` component I used on the home page. However, the app crashed. The strangest part is that I even copied the absolute import (\`import ToolCard from '@/components/ToolCard';\`) directly from the home page's TSX file, and the usage is exactly the same.
      ...
      <CodeBlock
        language="tsx"
        code={`// /components/layout/tool-recommend.tsx
    ...
    return (
        <div>
            ...
            <div className="flex flex-wrap gap-1">
                <ToolCard href="/" imgSrc="/icon.png"
                  imgAlt="Test" title="Test Card" />
            </div>
        </div>
    );
}`}
      ...
      <h2>The Mystery: Works Here, Fails There</h2>

      {md`
On my **homepage (\`/app/[locale]/page.tsx\`)**, I was using \`ToolCard\` directly. Everything worked perfectly.
...
Then, I created a layout for my tool pages, \`tool-layout.tsx\`. This layout would, in turn, render the \`ToolRecommend\` component.
      ...
      <CodeBlock
        language="tsx"
        code={`// /components/layout/tool-layout.tsx
...
**The failing import (relative path):**
      ...
      <CodeBlock
        language="tsx"
        code={`import ToolRecommend from "./tool-recommend";`}
      />
...
**The fix (absolute path alias):**
      ...
      <CodeBlock
        language="tsx"
        code={`import ToolRecommend from "@/components/layout/tool-recommend";`}
      />
